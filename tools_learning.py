...
def load_np_ema_and_mfcc_and_phone(filenames):
    """
    ...
    for filename in filenames:
        ...
        the_phone_file = np.load(os.path.join(files_path, "phone", filename + ".npy"))
        x.append(the_mfcc_file)
        y.append(the_ema_file)
        phone.append(the_phone_file)
    return x, y, phone
...
def criterion_pearson(y, y_pred, cuda_avail , device):
    ...
    """
    #new_y = torch.zeros(10,12)
    #new_ypred = torch.zeros(10,12)
    #for i in range(10):
    #    for j in range(12):
    #        new_y[i][j] = y[i][j]
    #        new_ypred[i][j] = y_pred[i][j]
...
def criterion_both(my_y,my_ypred,alpha,cuda_avail,device):
    ...
    #    for j in range(229):
    #        for k in range(12):
    #            new_y[i][j][k] = my_y[i][j][k]
    #my_y = my_y[:,:,0:12]
    #print(my_y.shape)
    #print(my_y)
    #my_ypred = my_ypred[:,:,0:12]
    #print(my_ypred.shape)
    #print(my_ypred)
    #a = alpha * criterion_pearson(my_y, my_ypred, cuda_avail, device)*multip
    b = torch.nn.MSELoss(reduction='mean')(my_y, my_ypred)
    #print(b)
    #print(b.shape)
    #new_loss = a + b
    return b

def criterion_rmse(my_y,my_ypred):
    #new_y = torch.zeros(10,12)
    #new_ypred = torch.zeros(10,12)
    #for i in range(10):
    #    for j in range(12):
    #        new_y[i][j] = my_y[i][j]
    #        new_ypred[i][j] = my_ypred[i][j]
    #my_y = my_y[:,:,0:12]
    #my_ypred = my_ypred[:,:,0:12]
    b = torch.nn.MSELoss(reduction='mean')(my_y, my_ypred)
    return b
...
def give_me_train_valid_test_filenames(train_on, test_on, config, batch_size, valid_on = []):
    ...
    if config == "spec":
        ...
        files_for_train = load_filenames(train_on, part=["train"])
        files_for_valid = load_filenames(train_on, part=["valid"])
        files_for_test = load_filenames([test_on], part=["test"])
