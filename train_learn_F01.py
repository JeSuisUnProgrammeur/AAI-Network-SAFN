...
def train_model(test_on, n_epochs, loss_train, patience, select_arti, corpus_to_train_on, batch_norma, filter_type,
    ...
    for epoch in range(n_epochs):
        ...
        for categ in categs_to_consider:
            ...
            while len(files_this_categ_courant) > 0: # go through all  the files batch by batch
                ...
                x,y  = model1.prepare_batch(x,y) 
                if cuda_avail:
                   x,y = x.to(device=model1.device).double(),y.to(device=model1.device).double()
                ...
                #print(x_torch.size())
                #print(emb.size())
                #new_input = torch.zeros((5,128,300),dtype=torch.double)
                #for i in range(5):
                #    for j in range(128):
                #        for k in range(300):
                #            new_input[i][j][k] = emb[i][j]
                ...
                #print(input.size())
                #input,y = model.prepare_batch(input,y)
                #input = input.to(device=model.device), y = y.to(device=model.device)
                y_pred = model(input)
                if cuda_avail:
        ...
        if epoch%delta_test == 0:  #toutes les delta_test epochs on évalue le modèle sur validation et on sauvegarde le modele si le score est meilleur
            ...
            for categ in categs_to_consider:  # de A à F pour le moment
                ...
                while len(files_this_categ_courant) >0 :
                    ...
                    x, y = model1.prepare_batch(x,y)
                    if cuda_avail:
                       x, y = x.to(device=model1.device), y.to(device=model1.device)
                    mu,log_sigma,emb,dec = model1(x)
                    eps = log_sigma.new(*log_sigma.size()).normal_(0,1)
                    x_torch = (mu+torch.exp(log_sigma/2)*eps)
                    emb = emb.unsqueeze(2)
    ...
    #L = len(x)
    #x_test,y_test = model1.prepare_batch()
    #x_torch = torch.from_numpy(x.view(1, L, 39))
    #print(x_torch.size())
    #x_torch = x_torch.cuda()
    #mu,log_sigma,emb,dec = model1(x_torch)
    #eps = log_sigma.new(*log_sigma.size()).normal_(0,1)
    #x_torch = (mu+torch.exp(log_sigma/2)*eps)
    #emb = emb.unsqueeze(2)
    #new_input = torch.cat([emb for i in range(0,300)],2)
